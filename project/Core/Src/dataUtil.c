...
//        itoa(begin, buffer, 10);
//        ST7735_WriteString(0, 40, buffer, MEDIUM_FONT, BLACK, WHITE);
//        HAL_Delay(SAMPLING_INTERVAL_MS);
//        memset(buffer, '\0', 1024);
//        itoa(end, buffer, 10);
//        ST7735_WriteString(0, 60, buffer, MEDIUM_FONT, BLACK, WHITE);
//        HAL_Delay(SAMPLING_INTERVAL_MS);
    ...
    memset(buffer, '\0', 1024);
    itoa(sum, buffer, 10);
    ST7735_WriteString(0, 80, buffer, MEDIUM_FONT, BLACK, WHITE);
    // -1 for excluding end and another -1 for excluding the datapoint
    (*avg_out).data_smoothened[(*avg_out).data_pointer] = sum / (end - begin - 2 > 0 ? end - begin - 2 : 1);
    memset(buffer, '\0', 1024);
    itoa(sum / (end - begin - 2), buffer, 10);
    ST7735_WriteString(0, SCREEN_H - 18, buffer, MEDIUM_FONT, BLACK, WHITE);
